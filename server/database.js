...
db.serialize(() => {
    ...
    /* SQL is something I don't understand super well yet so this may not be a good implementation, 
    but I think the matchHistory table should have a column where each row (match) can link to a 
    table of matchTicksHistory (below). Each matchTicksHistory table should have 40 (or however 
    many ticks we want per game) rows, and each row stores what happened in that tick. Thoughts?
    */
    ...
    */
    db.run(`CREATE TABLE IF NOT EXISTS matchTicksHistory (
        ...
        FOREIGN KEY (attempted_scorer_player_id) REFERENCES players(id)
    );`);

    // Maybe this table could store the combinations of players used and their roles
    // Could we store roles as an enum/ints?
    // It should hold references to targets as well
    // Maybe ticks 0-19 in a game refer to id 0, then ticks 20-39 refer to id 1 if a player was swapped
    // at halftime or their role changed
    // I left player 4 as optionally null because of the "huge" quirk
    db.run(`CREATE TABLE IF NOT EXISTS playedTeamHistory (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        player1_hist_id INT NOT NULL,
        player2_hist_id INT NOT NULL,
        player3_hist_id INT NOT NULL,
        player4_hist_id INT,
        FOREIGN KEY (player1_hist_id) REFERENCES playerHistory(id),
        FOREIGN KEY (player2_hist_id) REFERENCES playerHistory(id),
        FOREIGN KEY (player3_hist_id) REFERENCES playerHistory(id),
        FOREIGN KEY (player4_hist_id) REFERENCES playerHistory(id)
    );`);

    // Roles set as ints so we can maybe set them as an enum or something
    db.run(`CREATE TABLE IF NOT EXISTS playerHistory (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    // I'm actually not sure if damage done is an int, that may need to be corrected
    db.run(`CREATE TABLE IF NOT EXISTS attackHistory (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
